...
FontData * FNTReader::ReadFile(const char * fileName)
{
	...
	while (fntStream >> word)
	{
		...
		{
			FCharacterInfo characterInfo = ReadCharacterData(fntStream);
...
FCharacterInfo FNTReader::ReadCharacterData(ifstream & fntStream)
{
	...
	fntStream >> word;
	word = StripAfterEqualSign(word);
	uvRect.min.x = atof(word.c_str());

	// Y
	fntStream >> word;
	word = StripAfterEqualSign(word);
	uvRect.min.y = atof(word.c_str());

	// Width
	fntStream >> word;
	word = StripAfterEqualSign(word);
	uvRect.max.x = atof(word.c_str()) + uvRect.min.x;

	// Height
	fntStream >> word;
	word = StripAfterEqualSign(word);
	uvRect.max.y = atof(word.c_str()) + uvRect.min.y;
